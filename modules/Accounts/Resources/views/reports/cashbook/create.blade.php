...
@section('title', localize('cash_book'))
@push('css')
    <link href="{{ module_asset('Accounts/css/dailog.css?v_' . date('h_i')) }}" rel="stylesheet">
@endpush
@section('content')
    ...
    <div class="card mb-4 d-print-none fixed-tab-body">
        <div class="card-body ">
            <form id="leadForm" action="{{ route('reports.cashbookGenerate') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-3 col-xl-3 col-12">
                        ...
                        <select name="acc_coa_id" id="acc_coa_id" class="select-basic-single" required>
                            ...
                            @foreach ($accDropdown as $key => $accvalue)
    ...
    <div class="card mb-4 font-arial" id="print-table">
        ...
        <div class="card-body card-body-customize">
                    ...
                    <div class="row">
                        <div class="col-middle-6 pb-2">
                            ...
                            <h6><strong>{{ localize('total_balance') }} : </strong>
                                {{ bt_number_format($tablefooter['totalBalance']) ?? bt_number_format(0) }}</h6>
                        </div>
            ...
            <div class="table-responsive">
                <table id="cashbook-table" class="table display table-bordered w-100 align-middle">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @forelse($getTransactionList as $key => $data)
                            <tr>
                                ...
                                <td class="text-end">{{ bt_number_format($data->debit) ?? bt_number_format(0) }} </td>
                                <td class="text-end">{{ bt_number_format($data->credit) ?? bt_number_format(0) }}</td>
                                <td class="text-end">{{ bt_number_format($data->balance) ?? bt_number_format(0) }}</td>
                    ...
                    <tfoot>
                        <tr class="table_data">
                            ...
                            <td class="center">&nbsp;</td>
                            <td class="center">&nbsp;</td>
                            <td class="text-end"><strong class="text-dark">{{ localize('total') }}</strong></td>
                            <td class="text-end"><strong
                                    class="text-dark">{{ bt_number_format($tablefooter['totalDebit']) ?? bt_number_format(0) }}</strong>
                            </td>
                            <td class="text-end"><strong
                                    class="text-dark">{{ bt_number_format($tablefooter['totalCradit']) ?? bt_number_format(0) }}</strong>
                            </td>
                            <td class="text-end"><strong
                                    class="text-dark">{{ bt_number_format($tablefooter['totalBalance']) ?? bt_number_format(0) }}</strong>
                            </td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div class="card-body pt-0">
            ...
            <form id="download_pdf" action="{{ route('reports.cashbookPdf') }}" method="POST">
                @csrf
                <span class="d-print-none print-none export-cashbook"></span>
...
@endsection
@push('js')
    <script src="{{ module_asset('Accounts/js/customeacc.js?v_' . date('h_i')) }}"></script>
    <script src="{{ module_asset('Accounts/js/cash-book.js?v_' . date('h_i')) }}"></script>
@endpush
