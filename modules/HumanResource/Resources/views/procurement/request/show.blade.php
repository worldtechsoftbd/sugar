...
@section('title', localize('approval_request'))
@push('css')
@endpush
@section('content')
...
<div class="card mb-3 ">
    ...
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h6 class="fs-17 fw-semi-bold mb-0">{{ localize('approval_request') }}</h6>
            </div>
            <div class="text-end">
                <div class="actions">
    ...
    <div class="card-body">
        <div class="row justify-content-center">
            <div class="col-md-12 text-center">
                <form action="{{ route('procurement_request.approve', $req->id) }}" method="POST" class="f1" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                    ...
                                    <select name="employee_id" id="employee_id" class="form-select select-basic-single" required>
                                        ...
                                        @foreach ($employees as $key => $employee)
                                            <option value="{{ $employee->id }}"
                                                @if ($req->employee_id == $employee->id) selected @endif>
                                    ...
                                    <select name="department_id" id="department_id" class="form-select select-basic-single" required>
                                        ...
                                        @foreach ($departments as $key => $department)
                                            <option value="{{ $department->id }}"
                                                @if ($req->department_id == $department->id) selected @endif>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <label for="expected_start_date" class="col-form-label col-sm-3 col-md-12 col-xl-5 fw-semibold">{{ localize('expected_date_to_have_the_good_starts') }}
                                    <span class="text-danger">*</span></label>
                                <div class="col-sm-9 col-md-12 col-xl-7">
                                    <input type="text" name="expected_start_date" id="expected_start_date"  class="form-control date_picker" placeholder="{{ localize('select_date') }}" value="{{ $req->expected_start_date }}" required>
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <label for="expected_end_date" class="col-form-label col-sm-3 col-md-12 col-xl-5 fw-semibold">{{ localize('expected_date_to_have_the_good_ends') }}
                                    <span class="text-danger">*</span></label>
                                <div class="col-sm-9 col-md-12 col-xl-7">
                                    <input type="text" name="expected_end_date" id="expected_end_date"  class="form-control date_picker" placeholder="{{ localize('select_date') }}" value="{{ $req->expected_end_date }}" required>
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <label for="request_reason" class="col-form-label col-sm-3 col-md-12 col-xl-5 fw-semibold">{{ localize('reason_for_requesting') }}
                                    <span class="text-danger">*</span></label>
                                <div class="col-sm-9 col-md-12 col-xl-7">
                                    <textarea name="request_reason" id="request_reason" rows="2" class="form-control" placeholder="{{ localize('reason_for_requesting') }}" required>{{ $req->request_reason }}</textarea>
                                </div>
                    ...
                    <table class="table table-bordered table-hover" id="request_table">
                        ...
                        <tbody id="request_item">
                            @foreach ($req->requestItems as $request_item)
                            <tr>
                                <td width="50%">
                                    <textarea class="form-control" name="material_description[]" id="description" rows="2" placeholder="{{ localize('description_of_materials').' /'.localize('goods').' /'.localize('service') }}" required="">{{ $request_item->material_description }}</textarea>
                                </td>
                                <td width="20%">
                                ...
                                <td width="20%">
                                    <input type="number" class="form-control text-end" name="quantity[]" step="any" placeholder="0.00"  required  min="0" value="{{ $request_item->quantity }}"/>
                                </td>
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group row">
                                <label for="category_id" class="col-sm-3 col-form-label">{{ localize('reason_for_approval') }} <i class="text-danger">*</i></label>
                                <div class="col-sm-9">
                                    <textarea class="form-control" name="approval_reason" id="approval_reason" rows="2" placeholder="{{ localize('reason_for_approval') }}" required></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="form-group text-end">
                            <button type="submit" class="btn btn-success btn-lg">{{ localize('approve')}}</button>
                        </div>
                    </div>
                </form>
            </div>
...
<script src="{{ module_asset('HumanResource/js/request_show.js') }}"></script>
