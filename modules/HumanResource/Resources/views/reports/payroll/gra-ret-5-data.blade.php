    ...
    <div class="panel-body">
        ...
        <div class="text-right" id="print">
           <button type="button" class="btn btn-warning" id="btnPrint" onclick="printPageArea('printArea');"><i class="fa fa-print"></i></button>
            ...
            <a class="btn btn-success-soft" href="{{ $uuid != null?route('reports.gra-ret-5-pdf', $uuid):'#' }}"
                    ...
                    <tbody>
                        ...
                        @foreach ($gra_ret_5_data as $key => $row)
                            ...
                            <tr style="text-align: center;">
                                ...
                                <td>{{$setting->currency?->symbol.' '.$gross_salary}}</td>
                                <td>{{$setting->currency?->symbol.' '.$row->cumilative_gross_salary}}</td>
                                <td>{{$setting->currency?->symbol.' '.$income_tax}}</td>
                                <td>{{$setting->currency?->symbol.' '.$row->cumilative_income_tax}}</td>
                            </tr>
                        ...
                        <tr style="background-color: #c3bfbf;color: #000;text-align: center;font-weight: bold;font-size: 14px;">
                            <td>Totals</td>
                            <td></td>
                            <td>{{$setting->currency?->symbol.' '.$total_gross}}</td>
                            <td>{{$setting->currency?->symbol.' '.$total_cumulative_gross}}</td>
                            <td>{{$setting->currency?->symbol.' '.$total_state_income_tax}}</td>
                            <td>{{$setting->currency?->symbol.' '.$total_cumulative_state_income_tax}}</td>
                        </tr>
                    </tbody>
