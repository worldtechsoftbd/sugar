...
<div class="row ps-4 pe-4">
        ...
        <div class="row">
            <label for="summary"
                class="col-form-label col-sm-3 col-md-12 col-xl-3 fw-semibold">{{ localize('summary') }}<span
                    class="text-danger">*</span></label>
            <div class="col-sm-9 col-md-12 col-xl-9">
                <textarea  class="form-control" id="summary" name="summary" placeholder="{{ localize('summary') }}" rows ="3" required>{{ $backlog_task_data->summary }}</textarea>
            </div>
        ...
        <div class="row">
            <label for="description"
                class="col-form-label col-sm-3 col-md-12 col-xl-3 fw-semibold">{{ localize('description') }}<span
                    class="text-danger">*</span></label>
            <div class="col-sm-9 col-md-12 col-xl-9">
                <textarea  class="form-control" id="description" name="description" placeholder="{{ localize('description') }}" rows ="3" required>{{ $backlog_task_data->description }}</textarea>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                <input type="file" class="form-control" id="attachment" name="attachment"
                    ...
                    @if($backlog_task_data->attachment)
                        <a href="{{ asset('storage/' . $backlog_task_data->attachment) }}" target="_blank">{{ localize('attachment') }}</a>
                    @endif
                </div>
    ...
    <div class="col-md-12 mt-3">
        <div class="row">
            <label for="assignee"
                class="col-form-label col-sm-3 col-md-12 col-xl-3 fw-semibold">{{ localize('assignee') }}<span
                    class="text-danger">*</span></label>
            <div class="col-sm-9 col-md-12 col-xl-9">

                <select name="employee_id" id="assignee" class="form-control select-basic-single" placeholder=">{{ localize('select_assignee') }}" required>

                    @foreach ($team_members as $key => $team_member)
                        <option value="{{ $key }}" {{ $key == $backlog_task_data->employee_id ? 'selected' : '' }} >{{ $team_member }}
                        </option>
                    @endforeach
                </select>
            ...
            @if ($errors->has('assignee'))
                <div class="error text-danger m-2">{{ $errors->first('assignee') }}</div>
            @endif
        </div>
    </div>
    ...
    <div class="col-md-12 mt-3">
        <div class="row">
            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                ...
                <select name="sprint_id" id="sprint" class="form-control select-basic-single" placeholder=">{{ localize('select_sprint') }}">
                    @foreach ($available_sprints as $key => $available_sprint)
                        <option value="{{ $key }}" {{ $key == $backlog_task_data->sprint_id ? 'selected' : '' }} >{{ $available_sprint }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                ...
                <select name="priority" id="priority" class="form-control select-basic-single" placeholder=">{{ localize('select_priority') }}" required>
                    ...
                    <option value="2" {{ 2 == $backlog_task_data->priority ? 'selected' : '' }} >{{ localize('high') }}</option>
                    <option value="1" {{ 1 == $backlog_task_data->priority ? 'selected' : '' }} >{{ localize('low') }}</option>
                    <option value="0" {{ 0 == $backlog_task_data->priority ? 'selected' : '' }} >{{ localize('medium') }}</option>
                </select>
