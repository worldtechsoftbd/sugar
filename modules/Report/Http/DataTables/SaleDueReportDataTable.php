...
class SaleDueReportDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('n_status', function ($invoice) {

                $status = '';

                if ($invoice->partial_delivered == 1 && $invoice->all_delivered == 0) {
                    $status .= '<span class="badge badge-info-soft sale-badge-ft-13">Partial Delivered</span>';
                } else
                if ($invoice->partial_delivered == 0 && $invoice->all_delivered == 0) {
                    $status .= '<span class="badge badge-danger-soft sale-badge-ft-13">Not Delivered</span>';
                } else if ($invoice->all_delivered == 1 && $invoice->partial_delivered == 2) {
                    $status .= '<span class="badge badge-success-soft sale-badge-ft-13">All Delivered</span>';
                } else {
                    $status .= '<span class="badge badge-danger-soft sale-badge-ft-13">Not Delivered</span>';
                }
            ...
            ->addColumn('n_sale_type', function ($invoice) {
                $button = '';
                if ($invoice->sale_type == 1) {
                    return $button .= '<span class="badge badge-success-soft sale-badge-ft-13">Production</span>';
                } else {
                    return $button .= '<span class="badge badge-info-soft sale-badge-ft-13">Regular</span>';
                }
            })
            ...
            ->rawColumns(['n_status', 'invoice', 'n_sale_type', 'customer_name', 'action', 'total_amount', 'due_amount', 'paid_amount']);
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ->setTableId('sale-due-report-table')
            ...
            ->footerCallback('function ( row, data, start, end, display ) {
                       ...
                       var due_amount = api
                        ...
                        $(api.column( 2).footer() ).html("' . $span . '");
                        $(api.column( 3 ).footer() ).html(total);
                        $(api.column( 4 ).footer() ).html(paid_amount);
                        $(api.column( 5 ).footer() ).html(due_amount);
                    }')
                               ...
                               //add print date and time
                                $(win.document.body)
                                    .prepend(
                                        ...
                                        \'<h5 class="text-center">Sales Due Report</h5>\'+
