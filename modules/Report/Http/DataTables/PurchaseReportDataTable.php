...
class PurchaseReportDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('purchase_no', function ($purchase) {

                return '<a href="javascript:void(0)" onclick="detailsView(' . $purchase->id . ')" id="detailsView-' . $purchase->id . '" class="text-primary" data-url="' . route('report.purchase_details_show', $purchase->id) . '">' . $purchase->purchase_no . '</a>';
            })
            ->filterColumn('purchase_no', function ($query, $keyword) {
                $query->where('purchase_no', 'like', "%{$keyword}%");
            })

            ->addColumn('supplier_name', function ($purchase) {
                return '<a href="' . route('reports.redirect_sub_ledger_generate', ['acc_subcode_id' => $purchase->supplier_id, 'type' => 'supplier']) . '" target="_blank">' . ucwords($purchase->supplier->supplier_name) . '</a>';
            })
            ...
            ->editColumn('grand_total_amount', function ($purchase) {
                return bt_number_format($purchase->grand_total_amount);
            })
            ...
            ->rawColumns(['purchase_no', 'supplier_name', 'grand_total_amount', 'due_amount', 'paid_amount']);
                               ...
                               //add print date and time
                                $(win.document.body)
                                    .prepend(
                                        ...
                                        \'<h5 class="text-center">Purchase Details Report</h5>\'+
